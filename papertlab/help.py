...
from papertlab import __version__, utils
from papertlab.dump import dump  # noqa: F401
from papertlab.help_pats import exclude_website_pats
...
def get_package_files():
    for path in importlib_resources.files("papertlab.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def fname_to_url(filepath):
    ...
    if filepath.startswith("website/_includes/"):
        ...
        docid = "https://papert.in/" + filepath
...
def get_index():
    ...
    dname = Path.home() / ".papertlab" / "caches" / ("help." + __version__)
    ...
    if dname.exists():
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("papertlab.website")
                .joinpath(fname)
