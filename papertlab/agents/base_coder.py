...
from papertlab.io import ConfirmGroup, InputOutput
...
from papertlab.utils import format_content, format_messages, format_tokens, is_image_file
...
class Coder:
    ...
    def create(
        ...
        **kwargs,
        ...
        import papertlab.agents as agents

        if edit_format == "autopilot":
            return agents.AutopilotCoder(main_model, io, from_coder=from_coder, **kwargs)
    ...
    def __init__(
        ...
        self.refresh_readonly_files()

    def refresh_readonly_files(self):
        if self.repo:
            files_to_remove = self.repo.refresh_readonly_spec()
            
            for file in self.get_all_relative_files():
                if self.repo.is_readonly(file):
    ...
    def init_before_message(self):
        self.papertlab_edited_files = set()
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.papertlab_edited_files.update(edited)
    ...
    def calculate_and_show_tokens_and_cost(self, messages, completion=None):
        ...
        self.input_token = self.message_tokens_sent
        self.output_token = self.message_tokens_received
    ...
    def allowed_to_edit(self, path):
        ...
        if full_path in self.abs_read_only_fnames or (self.repo and self.repo.is_readonly(path)):
            return False
        if self.repo:
    ...
    def auto_commit(self, edited, context=None):
        ...
        if not self.repo or not result or self.dry_run:
            return
    ...
    def show_auto_commit_outcome(self, res):
        ...
        self.git_commit_id = f"You can use /undo to undo and discard each papertlab commit id: {commit_hash}."
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head():
            self.io.tool_output("You can use /undo to undo and discard each papertlab commit.")
