...
class Coder:
    ...
    def add_rel_fname(self, rel_fname):
        if self.repo and self.repo.is_readonly(rel_fname):
            abs_fname = self.abs_root_path(rel_fname)
            self.abs_read_only_fnames.add(abs_fname)
            self.io.tool_output(f"Added {rel_fname} as read-only file.")
        else:
    ...
    def check_for_urls(self, inp):
        ...
        for url in urls:
            if url not in self.rejected_urls:
                if self.io.confirm_ask(f"Add URL to the chat?", subject=url, group=group):
                    inp += "\n\n"
