...
def check_gitignore(git_root, io, ask=True):
    ...
    pat = ".papertlab.chat.history.md"
    pat_cache = ".papertlab.tags.cache.v3"
    pat_db = "papertlab_gui.db"
    ...
    if gitignore_file.exists():
        ...
        if pat in content.splitlines():
            return
        if pat_cache in content.splitlines():
            return
        if pat_db in content.splitlines():
            return
    else:
        content = ""

    if ask and not io.confirm_ask(f"Add {pat}, {pat_cache} and {pat_db} to .gitignore (recommended)?"):
        return
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    # Create .papertlabignore file if it doesn't exist
    if git_root:
        create_papertlabignore(git_root)
        create_papertlab_readonly(git_root)    
